...
'''
            ...
            k = 1
            while k <= speed:
                n_row = shark.row + dy[shark.direction]
                n_column = shark.column + dx[shark.direction]

                if not (0 <= n_row < r and 0 <= n_column < c): #다음 위치가 벽일 때
                    shark.reverse_direction()
                    n_row = shark.row + dy[shark.direction]
                    n_column = shark.column + dx[shark.direction]
                shark.row = n_row
                shark.column = n_column
...
'''
def test(r, c, row, column, speed, direction):
    # 스피드만큼 반복
    n_row = row + (dy[direction] * speed)
    n_column = column + (dx[direction] * speed)
